...
export class AdminOrdersComponent implements OnInit {
  ...
  async load(): Promise<void> {
    ...
    this.error.set('');
    try {
      this.rows.set(await this.svc.listOrders());
    } catch (e: any) {
      this.error.set(e?.message || 'Failed to load institutional orders.');
    } finally {
  ...
  async save(r: AdminLeadRow): Promise<void> {
    ...
    this.error.set('');
    try {
      await this.svc.updateOrder(r.id, { status: r.status, admin_notes: r.admin_notes });
