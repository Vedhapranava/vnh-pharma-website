...
import { AdminEnquiriesService, AdminLeadRow } from '../enquiries/admin-enquiries.service';
...
export class AdminPartnersComponent implements OnInit {
  ...
  async load(): Promise<void> {
    ...
    this.error.set('');
    try {
      this.rows.set(await this.svc.listPartners());
    } catch (e: any) {
      this.error.set(e?.message || 'Failed to load partner enquiries.');
    } finally {
  ...
  async save(r: AdminLeadRow): Promise<void> {
    ...
    this.error.set('');
    try {
      await this.svc.updatePartner(r.id, { status: r.status, admin_notes: r.admin_notes });
